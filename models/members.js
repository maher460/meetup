...
mongoClient.connect('mongodb://'+connection_string, function(err, db) {
  ...
  db.collection("members").createIndex( { "group_key": 1, "user_id": 1 }, { unique: true } )
});
 ...
 */
exports.create = function(group_key, user_id, longitude, latitude, callback) {
  console.log("4. Start insert function in members");

  mongoDB.collection("members").updateOne(

    {"group_key": group_key, "user_id": user_id},                   // Filter selects which documents to update
      {"$set":{"longitude":longitude, "latitude": latitude}},                   // The update operation
      {upsert:true},    

    function(err, status) {  
      if (err) doError(err);
      console.log("5. Done with mongo insert operation in members");
      var success = (status.result.n == 1 ? true : false);
      callback(success);
      console.log("6. Done with insert operation callback in members");
    });

  console.log("7. Done with insert function in members");
}
...
exports.retrieve = function(group_key, callback) {
  mongoDB.collection("members").find({"group_key" : group_key}).toArray(function(err, docs) {
    if (err) doError(err);
    callback(docs);
  });
}

exports.retrieve_one = function(group_key, user_id, callback) {
  mongoDB.collection("members").find({"group_key" : group_key, "user_id" : user_id}).toArray(function(err, docs) {
    if (err) doError(err);
    callback(docs);
  });
}
 ...
 */
exports.update = function(group_key, user_id, longitude, latitude, callback) {
  mongoDB
    .collection("members")     // The collection to update
...
exports.delete = function(group_key, user_id, callback) {
  mongoDB
    .collection("members")
    .deleteMany(
      {"group_key": group_key, "user_id": user_id},
      function(err, status) {   // Callback upon error or success
        if (err) doError(err);
